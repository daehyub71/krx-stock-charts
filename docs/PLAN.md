...
| 진행 중인 주/월 봉 | 마지막 주/월봉이 낡은 값으로 고정 | 증분 갱신 시 마지막 주봉·월봉 **재계산 후 덮어쓰기** (§3). 덮어쓰기가 성립하려면 키가 고정이어야 한다 — **구간 시작일**을 쓴다 (D9) |
...
| 2026-09-23 | SPEC v2.6(D9) 반영 — 주·월봉 키를 구간 시작일로. 리샘플 규칙과 전량 재생성 절차 |
