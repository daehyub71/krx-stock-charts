...
- **지수 코드 체계를 헷갈리지 마라** — pykrx는 코스피 `1001`·코스닥 `2001`이고,
  KIS는 코스피 `0001`·코스닥 `1001`이다. **`1001`이 두 체계에서 다른 시장**이라,
  KIS 문서를 보고 `0001`을 쓰면 pykrx에서 **0행**이 오고 `1001`을 코스피로 알고 쓰면 맞다.
  2026-09-05 이름 조회로 확인: `1001`→코스피 · `2001`→코스닥 · `0001`→없음.
- **KRX 로그인이 없으면 pykrx가 예외 없이 0행을 준다** — 휴장일과 겉보기가 같다 (2026-09-05 실측).
  거래일인데 0행이면 소리를 낸다. `try/except`로는 절대 안 걸린다.
- **지수는 `ksc_bars`에 못 넣는다** — 그 표는 `ksc_tickers` FK와 `^[0-9A-Z]{6}$` 제약이 있다.
  그리고 **가격이 `int`다** — 지수는 6,579.48처럼 소수점이 있어 0.5% 오차가 하위의
  초과수익 계산에 그대로 실린다. `ksc_index_bars`는 `numeric(12,2)`다.
- **`upsert`는 `updated_at`을 안 건드린다** — `default now()`는 INSERT에만 걸린다.
  `ksc_meta.updated_at`이 그래서 **행이 처음 만들어진 시각(2026-08-15)에 멈춰 있었다**
  (자료는 2026-09-04까지 있었는데). 하위가 발견해 2026-09-05에 고쳤다:
  `store.set_meta`가 값을 넘기고, DB 트리거도 함께 둔다(어느 writer가 와도 맞게).
